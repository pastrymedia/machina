 ...
 * for the Machina Simple Sidebars plugin.
 *
 * @since 1.0.0
 */
class Machina_Simple_Sidebars_Admin extends Machina_Admin_Basic {
	 ...
	 *
	 * @see Machina_Admin_Import_Export::actions() Handle creating, editing, and deleting sidebars.
	 */
	public function __construct() {
		...
		$menu_ops = array(
			'submenu' => array(
				'parent_slug' => 'machina',
				'page_title'  => __( 'Machina - Simple Sidebars', 'ss' ),
		...
		$settings_field = MACHINA_SIDEBAR_SETTINGS_FIELD;
	...
	public function admin() {
		...
		echo '<div class="wrap">';
			...
			if ( isset( $_REQUEST['action'] ) && 'edit' == $_REQUEST['action'] )
				require_once( MACHINA_SIDEBARS_DIR . '/includes/views/edit.php' );
			else
				require_once( MACHINA_SIDEBARS_DIR . '/includes/views/main.php' );
	...
	public function actions() {
		...
		if ( ! machina_is_menu_page( 'simple-sidebars' ) )
			return;
	...
	protected function create_sidebar( $args = array() ) {
		...
		$db = (array) get_option( MACHINA_SIDEBAR_SETTINGS_FIELD );
		$new = array(
		...
		update_option( MACHINA_SIDEBAR_SETTINGS_FIELD, $_sidebars );
	...
	protected function delete_sidebar( $id = '' ) {
		...
		$_sidebars = (array) get_option( MACHINA_SIDEBAR_SETTINGS_FIELD );
...
add_action( 'machina_admin_menu', 'simplesidebars_settings_menu' );
/**
...
function simplesidebars_settings_menu() {
	...
	new Machina_Simple_Sidebars_Admin;
