...
abstract class Machina_Admin {
	...
	public function create( $page_id = '', array $menu_ops = array(), array $page_ops = array(), $settings_field = '', array $default_settings = array() ) {
		...
		if ( isset( $this->menu_ops['submenu'] ) && ( isset( $this->menu_ops['main_menu'] ) || isset( $this->menu_ops['first_submenu'] ) ) )
			wp_die( sprintf( __( 'You cannot use %s to create two menus in the same subclass. Please use separate subclasses for each menu.', 'machina' ), 'Machina_Admin' ) );
	...
	public function maybe_add_main_menu() {
		...
		if ( isset( $this->menu_ops['main_menu']['sep'] ) ) {
			...
			if ( $sep['sep_position'] && $sep['sep_capability'] )
				$GLOBALS['menu'][$sep['sep_position']] = array( '', $sep['sep_capability'], 'separator', '', 'machina-separator wp-menu-separator' );
		}
	...
	public function register_settings() {
		...
		if ( machina_get_option( 'reset', $this->settings_field ) ) {
			if ( update_option( $this->settings_field, $this->default_settings ) )
				machina_admin_redirect( $this->page_id, array( 'reset' => 'true' ) );
			else
				machina_admin_redirect( $this->page_id, array( 'error' => 'true' ) );
			exit;
		}
	 ...
	 *
	 * Within Machina pages, the id attributes of form fields are the same as
	...
	protected function get_field_value( $key ) {
		...
		return machina_get_option( $key, $this->settings_field );
...
/**
 * Abstract subclass of Machina_Admin which adds support for displaying a form.
 *
 ...
 * @package Machina\Admin
 */
abstract class Machina_Admin_Form extends Machina_Admin {
	...
	public function admin() {
		...
		<form method="post" action="options.php">
			...
			<p class="top-buttons">
				...
				submit_button( $this->page_ops['reset_button_text'], 'secondary machina-js-confirm-reset', $this->get_field_name( 'reset' ), false, array( 'id' => '' ) );
				?>
			</p>
...
/**
 * Abstract subclass of Machina_Admin which adds support for registering and
 ...
 * @package Machina\Admin
 */
abstract class Machina_Admin_Boxes extends Machina_Admin {
	...
	public function admin() {
		...
		?>
		<div class="wrap machina-metaboxes">
		<form method="post" action="options.php">
	...
	public function do_metaboxes() {
		...
		<div class="metabox-holder">
			<div class="postbox-container">
				<?php
				do_action( 'machina_admin_before_metaboxes', $this->pagehook );
				do_meta_boxes( $this->pagehook, 'main', null );
				if ( isset( $wp_meta_boxes[$this->pagehook]['column2'] ) )
					do_meta_boxes( $this->pagehook, 'column2', null );
				do_action( 'machina_admin_after_metaboxes', $this->pagehook );
				?>
			</div>
		</div>
		<?php
...
/**
 * Abstract subclass of Machina_Admin which adds support for creating a basic
 ...
 * @package Machina\Admin
 */
abstract class Machina_Admin_Basic extends Machina_Admin {

	/**
	 * Satisfies the abstract requirements of Machina_Admin.
	 *
