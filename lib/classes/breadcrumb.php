 ...
 * If you change a private property expect that change to break Machina in the future.
 *
 * @since 1.5.0
 *
 * @package Machina\Breadcrumbs
 */
class Machina_Breadcrumb {
	...
	public function __construct() {
		...
		//* Default arguments
		$this->args = array(
			'home'                    => __( 'Home', 'machina' ),
			'sep'                     => ' / ',
			'list_sep'                => ', ',
			'prefix'                  => sprintf( '<div %s>', machina_attr( 'breadcrumb' ) ),
			'suffix'                  => '</div>',
			'heirarchial_attachments' => true,
			'heirarchial_categories'  => true,
			'labels' => array(
				'prefix'    => __( 'You are here: ', 'machina' ),
				'author'    => __( 'Archives for ', 'machina' ),
				'category'  => __( 'Archives for ', 'machina' ),
				'tag'       => __( 'Archives for ', 'machina' ),
				'date'      => __( 'Archives for ', 'machina' ),
				'search'    => __( 'Search for ', 'machina' ),
				'tax'       => __( 'Archives for ', 'machina' ),
				'post_type' => __( 'Archives for ', 'machina' ),
				'404'       => __( 'Not found: ', 'machina' )
			)
		);
	...
	public function get_output( $args = array() ) {
		...
		$this->args = apply_filters( 'machina_breadcrumb_args', wp_parse_args( $args, $this->args ) );
	...
	protected function build_crumbs() {
		...
		$crumbs = apply_filters( 'machina_build_crumbs', $crumbs, $this->args );
	...
	protected function get_archive_crumb() {
		...
		return apply_filters( 'machina_archive_crumb', $crumb, $this->args );
	...
	protected function get_single_crumb() {
		...
		return apply_filters( 'machina_single_crumb', $crumb, $this->args );
	...
	protected function get_home_crumb() {
		...
		$crumb = ( is_home() && is_front_page() ) ? $this->args['home'] : $this->get_breadcrumb_link( $url, sprintf( __( 'View %s', 'machina' ), $this->args['home'] ), $this->args['home'] );

		return apply_filters( 'machina_home_crumb', $crumb, $this->args );
	...
	protected function get_blog_crumb() {
		...
		return apply_filters( 'machina_blog_crumb', $crumb, $this->args );
	...
	protected function get_search_crumb() {
		...
		return apply_filters( 'machina_search_crumb', $crumb, $this->args );
	...
	protected function get_404_crumb() {
		...
		return apply_filters( 'machina_404_crumb', $crumb, $this->args );
	...
	protected function get_page_crumb() {
				...
				foreach ( $ancestors as $ancestor ) {
					array_unshift(
						...
						$this->get_breadcrumb_link(
							...
							sprintf( __( 'View %s', 'machina' ), get_the_title( $ancestor ) ),
		...
		return apply_filters( 'machina_page_crumb', $crumb, $this->args );
	...
	protected function get_attachment_crumb() {
		...
		if ( $this->args['heirarchial_attachments'] ) {
			...
			$crumb = $this->get_breadcrumb_link(
				...
				sprintf( __( 'View %s', 'machina' ), $attachment_parent->post_title ),
			...
			if ( ! $this->args['heirarchial_categories'] ) {
				...
				foreach ( $categories as $category ) {
					$crumbs[] = $this->get_breadcrumb_link(
						...
						sprintf( __( 'View all posts in %s', 'machina' ), $category->name ),
						$category->name
					);
				}
	...
	protected function get_cpt_crumb() {
		...
		if ( $cpt_archive_link = get_post_type_archive_link( $post_type ) )
			$crumb = $this->get_breadcrumb_link( $cpt_archive_link, sprintf( __( 'View all %s', 'machina' ), $post_type_object->labels->name ), $post_type_object->labels->name );
		else
	...
	protected function get_category_crumb() {
		...
		return apply_filters( 'machina_category_crumb', $crumb, $this->args );
	...
	protected function get_tag_crumb() {
		...
		return apply_filters( 'machina_tag_crumb', $crumb, $this->args );
	...
	protected function get_tax_crumb() {
		...
		return apply_filters( 'machina_tax_crumb', $crumb, $this->args );
	...
	protected function get_year_crumb() {
		...
		return apply_filters( 'machina_year_crumb', $crumb, $this->args );
	...
	protected function get_month_crumb() {
		...
		$crumb = $this->get_breadcrumb_link( get_year_link( $year ),
			sprintf( __( 'View archives for %s', 'machina' ), $year ),
			$year,
			$this->args['sep']
		);
		$crumb .= $this->args['labels']['date'] . single_month_title( ' ', false );
		return apply_filters( 'machina_month_crumb', $crumb, $this->args );
	...
	protected function get_day_crumb() {
		...
		$crumb .= $this->get_breadcrumb_link(
			...
			sprintf( __( 'View archives for %s %s', 'machina' ), $wp_locale->get_month( $month ), $year ),
			$wp_locale->get_month( $month ),
			$this->args['sep']
		);
		$crumb .= $this->args['labels']['date'] . $day . date( 'S', mktime( 0, 0, 0, 1, $day ) );
		return apply_filters( 'machina_day_crumb', $crumb, $this->args );
	...
	protected function get_author_crumb() {
		...
		return apply_filters( 'machina_author_crumb', $crumb, $this->args );
	...
	protected function get_post_type_crumb() {
		...
		return apply_filters( 'machina_post_type_crumb', $crumb, $this->args );
	...
	protected function get_term_parents( $parent_id, $taxonomy, $link = false, array $visited = array() ) {
		...
		if ( $link && !is_wp_error( get_term_link( get_term( $parent->term_id, $taxonomy ), $taxonomy ) ) ) {
			$chain[] = $this->get_breadcrumb_link( get_term_link( get_term( $parent->term_id, $taxonomy ), $taxonomy ), sprintf( __( 'View all items in %s', 'machina' ), $parent->name ), $parent->name );
		} else {
			$chain[] = $parent->name;
		}
	...
	protected function get_breadcrumb_link( $url, $title, $content, $sep = false ) {
		...
		$link = apply_filters( 'machina_breadcrumb_link', $link, $url, $title, $content, $this->args );
