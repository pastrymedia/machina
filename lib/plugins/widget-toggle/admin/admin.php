 ...
 * @since 0.2
 */
class GWT_Admin_settings extends Machina_Admin_Boxes {
	...
	function __construct() {
		/** unique page ID for Machina Admin */
		$page_id = 'machina-widget-toggle';
		/** define the menu and page titles */
		$menu_ops = array(
			'submenu' => array(
				'parent_slug' => 'machina',
				'page_title'  => __( 'Machina Widget Toggle Settings', 'widgettoggle' ),
				'menu_title'  => __( 'Widget Toggle', 'widgettoggle' ),
			)
		);
		/** Machina Widget Toggle Setting Fields */
		$settings_field = GWT_SETTINGS;
 		/** Machina Page Opt */
		$page_ops = array();
		/** Machina Widget Toggle default values */
	...
	function general_settings(){
		...
		<p><strong><?php _e( 'Support', 'widgettoggle' );?> : </strong><?php echo sprintf( __( 'Feedback, questions or suggestion just visit %sWordPress plugin support%s page.', 'widgettoggle' ) , '<a target="_blank" href="http://wordpress.org/support/plugin/machina-widget-toggle">', '</a>' )?></p>
