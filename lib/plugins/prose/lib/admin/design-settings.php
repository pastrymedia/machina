 ...
 * @since 1.5.0
 */
class Prose_Design_Settings extends Machina_Admin_Boxes {
	 ...
	 * @since 1.5.0
	 *
	 * @uses MACHINA_DESIGN_SETTINGS_FIELD settings field key
	 *
	 */
	function __construct() {
		...
		$settings_field = MACHINA_DESIGN_SETTINGS_FIELD;
	...
	function notices() {
		...
		if ( ! machina_is_menu_page( $this->page_id ) )
			return;

		/** Machina_Admin notices */
		parent::notices();
	...
	function metaboxes() {
		...
		/** Hidden form fields */
		add_action( 'machina_admin_before_metaboxes', array( $this, 'hidden_fields' ) );
...
add_action( 'machina_admin_menu', 'prose_design_settings_menu' );
/**
...
function prose_design_settings_menu() {
	...
	if( ! current_theme_supports( 'machina-design-settings' ) )
		return;
