...
class DDW_GWNF_Search_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		$widget_options = apply_filters( 'gwnf_filter_search_widget_options', array(
			...
			'description' => esc_html__( 'A search form for your site. With a more options than the default one.', 'machina-widgetized-notfound' ) . ' &ndash; ' . sprintf( esc_html__( 'Provided by the plugin %s.', 'machina-widgetized-notfound' ), '*' . __( 'Machina Widgetized Not Found & 404', 'machina-widgetized-notfound' ) . '*' ),
		) );
		...
		parent::__construct(
			'gwnf-search-widget',
			__( 'Machina - Search Form', 'machina-widgetized-notfound' ),
			$widget_options,
			$control_options
		);
	...
	public function form( $instance ) {
		...
		$defaults = apply_filters( 'gwnf_filter_search_widget_defaults', array(
			...
			'title_url'        => '',
			'title_url_target' => 0,
			'label_text'       => apply_filters( 'machina_search_form_label', '' ),
			'placeholder_text' => apply_filters( 'machina_search_text', esc_attr__( 'Search this website', 'machina-widgetized-notfound' ) . '&#x02026;' ),
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>">
				<strong><?php _e( 'Title:', 'machina-widgetized-notfound' ); ?></strong>
			</label>
		...
		<p>
			...
			<?php _e( 'Optional Title URL:', 'machina-widgetized-notfound' ); ?>
			</label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'title_url_target' ); ?>">
				<?php _e( 'Open the URL in a new window/ tab?' , 'machina-widgetized-notfound' ); ?>
			</label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'hide_title' ); ?>">
				<?php _e( 'Do not display the Title?' , 'machina-widgetized-notfound' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php /** Optional intro text */ echo $this->get_field_id( 'intro_text' ); ?>"><?php _e( 'Optional intro text:', 'machina-widgetized-notfound' ); ?>
				<small><?php echo sprintf( __( 'Add some additional %s info. NOTE: Just leave blank to not use at all.', 'machina-widgetized-notfound' ), __( 'Search', 'machina-widgetized-notfound' ) ); ?></small>
		...
		<p>
			<strong><?php _e( 'Search form', 'machina-widgetized-notfound' ); ?>:</strong>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'label_text' ); ?>">
			<?php _e( 'Label string before search input field:', 'machina-widgetized-notfound' ); ?>
			<input type="text" id="<?php echo $this->get_field_id( 'label_text' ); ?>" name="<?php echo $this->get_field_name( 'label_text' ); ?>" value="<?php echo esc_attr( $instance[ 'label_text' ] ); ?>" class="widefat" />
				<small><?php _e( 'NOTE: Leave empty to not use/ display this string!', 'machina-widgetized-notfound' ); ?></small>
			</label>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'placeholder_text' ); ?>">
			<?php _e( 'Placeholder string for search input field:', 'machina-widgetized-notfound' ); ?>
		...
		<p>
			...
			<?php _e( 'Search button string:', 'machina-widgetized-notfound' ); ?>
			<input type="text" id="<?php echo $this->get_field_id( 'button_text' ); ?>" name="<?php echo $this->get_field_name( 'button_text' ); ?>" value="<?php echo esc_attr( $instance[ 'button_text' ] ); ?>" class="widefat" />
				<small><?php _e( 'NOTE: Displaying may depend on your child theme settings/ styles.', 'machina-widgetized-notfound' ); ?></small>
			</label>
		</p>
		...
		<p>
    		<label for="<?php echo $this->get_field_id( 'widget_display' ); ?>">
				<strong><?php _e( 'Where to display this widget?', 'machina-widgetized-notfound' ); ?>:</strong>
				<select id="<?php echo $this->get_field_id( 'widget_display' ); ?>" name="<?php echo $this->get_field_name( 'widget_display' ); ?>">
					<?php
						printf( '<option value="global" %s>%s</option>', selected( 'global', $instance[ 'widget_display' ], 0 ), __( 'Global (default)', 'machina-widgetized-notfound' ) );

						echo $gwnf_select_divider;

						printf( '<option value="single_posts" %s>%s</option>', selected( 'single_posts', $instance[ 'widget_display' ], 0 ), sprintf( __( 'Single %s', 'machina-widgetized-notfound' ), __( 'Posts', 'machina-widgetized-notfound' ) ) );

						printf( '<option value="single_pages" %s>%s</option>', selected( 'single_pages', $instance[ 'widget_display' ], 0 ), sprintf( __( 'Single %s', 'machina-widgetized-notfound' ), __( 'Pages', 'machina-widgetized-notfound' ) ) );

						printf( '<option value="single_posts_pages" %s>%s</option>', selected( 'single_posts_pages', $instance[ 'widget_display' ], 0 ), __( 'Both, Single Posts & Pages', 'machina-widgetized-notfound' ) );
					?>
				</select>
        	</label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'not_in_public' ); ?>">
				<?php _e( 'Only displaying for logged in users?' , 'machina-widgetized-notfound' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php /** Optional outro text */ echo $this->get_field_id( 'outro_text' ); ?>"><?php _e( 'Optional outro text:', 'machina-widgetized-notfound' ); ?>
				<small><?php echo sprintf( __( 'Add some additional %s info. NOTE: Just leave blank to not use at all.', 'machina-widgetized-notfound' ), __( 'Search', 'machina-widgetized-notfound' ) ); ?></small>
