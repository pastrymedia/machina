...
/** Load everything */
add_action( 'machina_meta', 'load_gmie_banner_image' );

function load_gmie_banner_image() {

	/** Remove default minimum image if it exists */
	$custom_image_id = machina_get_custom_field( '_gmie_image_id' );
        if ( $custom_image_id ) {
		remove_action( 'machina_after_header', 'minimum_featured_image' );

		/** Add the new featured & custom image sections */
		add_action( 'machina_after_header', 'gmie_banner_image' );
	}
