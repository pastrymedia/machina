...
function ss_add_inpost_metabox() {
	...
	foreach ( (array) get_post_types( array( 'public' => true ) ) as $type ) {
		...
		if ( post_type_supports( $type, 'machina-simple-sidebars' ) || $type == 'post' || $type == 'page' ) {
	...
	<p>
		...
		<select name="_ss_sidebar" id="_ss_sidebar" style="width: 99%">
			...
			foreach ( (array) $_sidebars as $id => $info ) {
				printf( '<option value="%s" %s>%s</option>', esc_html( $id ), selected( $id, machina_get_custom_field( '_ss_sidebar' ), false), esc_html( $info['name'] ) );
			}
			?>
		</select>
	</p>
...
?>
	<p>
		...
		<select name="_ss_sidebar_alt" id="_ss_sidebar_alt" style="width: 99%">
			...
			<?php
			foreach ( (array) $_sidebars as $id => $info ) {
				printf( '<option value="%s" %s>%s</option>', esc_html( $id ), selected( $id, machina_get_custom_field( '_ss_sidebar_alt' ), false ), esc_html( $info['name'] ) );
			}
			?>
		</select>
	</p>
