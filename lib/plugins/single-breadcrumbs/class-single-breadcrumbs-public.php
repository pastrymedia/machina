 ...
 * @package Machina_Single_Breadcrumbs
 */
class Machina_Single_Breadcrumbs_Public {
	...
	private function __construct() {
		add_action( 'machina_before', array( $this, 'disable_breadcrumbs' ) );
		add_filter( 'machina_single_crumb', array( $this, 'filter_single_crumb' ), 10, 2 );
		add_filter( 'machina_page_crumb', array( $this, 'filter_page_crumb' ), 10, 2 );
	}
	...
	/**
	 * Disable the breadcrumbs, by unhooking machina_do_breadcrumbs from whereever it is currently hooked.
	 *
	 * @since 1.0.0
	 *
	 * @uses Machina_Single_breadcrumbs_Public::get_hook_for_callback()
	 */
	public function disable_breadcrumbs() {
		if ( is_singular() && machina_get_custom_field( '_machina_single_breadcrumbs_disable' ) ) {
			$hook_data = $this->get_hook_for_callback( 'machina_do_breadcrumbs' );
			remove_action( $hook_data['hook'], 'machina_do_breadcrumbs', $hook_data['priority'] );
		}
	}
	...
	protected function get_breadcrumb_title( $post_id = null ) {
		...
		$custom_field = get_post_meta( $post_id, '_machina_single_breadcrumbs_title', true );
		...
		foreach ( $ancestors as $ancestor ) {
			array_unshift(
				...
				$this->get_breadcrumb_link(
					...
					sprintf( __( 'View %s', 'machina-single-breadcrumbs' ), get_the_title( $ancestor ) ),
	 ...
	 *
	 * Taken straight from Machina 2.0.1, but with an extra parameter, so that the args can be passed through to the
	 * filter, as in Machina core.
	 *
	 * @since 1.1.0
	 *
	...
	protected function get_breadcrumb_link( $url, $title, $content, $sep = false, $args ) {
		...
		$link = apply_filters( 'machina_breadcrumb_link', $link, $url, $title, $content, $args );
