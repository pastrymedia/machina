...
add_filter( 'wp_title', 'machina_doctitle_wrap', 20 );
/**
 ...
 */
function machina_doctitle_wrap( $title ) {

	return is_feed() || is_admin() ? $title : sprintf( "<title>%s</title>\n", $title );

}

add_action( 'machina_title', 'wp_title' );
add_filter( 'wp_title', 'machina_default_title', 10, 3 );
/**
 ...
 */
function machina_default_title( $title, $sep, $seplocation ) {
	...
	$sep = machina_get_seo_option( 'doctitle_sep' ) ? machina_get_seo_option( 'doctitle_sep' ) : '–';
	$seplocation = machina_get_seo_option( 'doctitle_seplocation' ) ? machina_get_seo_option( 'doctitle_seplocation' ) : 'right';

	//* If viewing the home page
	if ( is_front_page() ) {
		//* Determine the doctitle
		$title = machina_get_seo_option( 'home_doctitle' ) ? machina_get_seo_option( 'home_doctitle' ) : get_bloginfo( 'name' );

		//* Append site description, if necessary
		$title = machina_get_seo_option( 'append_description_home' ) ? $title . " $sep " . get_bloginfo( 'description' ) : $title;
	}

	//* if viewing a post / page / attachment
	if ( is_singular() ) {
		//* The User Defined Title (Machina)
		if ( machina_get_custom_field( '_machina_title' ) )
			$title = machina_get_custom_field( '_machina_title' );
		//* All-in-One SEO Pack Title (latest, vestigial)
		elseif ( machina_get_custom_field( '_aioseop_title' ) )
			$title = machina_get_custom_field( '_aioseop_title' );
		//* Headspace Title (vestigial)
		elseif ( machina_get_custom_field( '_headspace_page_title' ) )
			$title = machina_get_custom_field( '_headspace_page_title' );
		//* Thesis Title (vestigial)
		elseif ( machina_get_custom_field( 'thesis_title' ) )
			$title = machina_get_custom_field( 'thesis_title' );
		//* SEO Title Tag (vestigial)
		elseif ( machina_get_custom_field( 'title_tag' ) )
			$title = machina_get_custom_field( 'title_tag' );
		//* All-in-One SEO Pack Title (old, vestigial)
		elseif ( machina_get_custom_field( 'title' ) )
			$title = machina_get_custom_field( 'title' );
	}
	...
	if ( is_post_type_archive() && machina_has_post_type_archive_support() ) {
		$title = machina_get_cpt_option( 'doctitle' ) ? machina_get_cpt_option( 'doctitle' ) : $title;
	}

	//* If we don't want site name appended, or if we're on the home page
	if ( ! machina_get_seo_option( 'append_site_title' ) || is_front_page() )
...
add_action( 'get_header', 'machina_doc_head_control' );
/**
 ...
 * @uses machina_get_seo_option() Get SEO setting value
 */
function machina_doc_head_control() {

	remove_action( 'wp_head', 'wp_generator' );

	if ( ! machina_get_seo_option( 'head_adjacent_posts_rel_link' ) )
		remove_action( 'wp_head', 'adjacent_posts_rel_link_wp_head', 10, 0 );

	if ( ! machina_get_seo_option( 'head_wlwmanifest_link' ) )
		remove_action( 'wp_head', 'wlwmanifest_link' );

	if ( ! machina_get_seo_option( 'head_shortlink' ) )
		remove_action( 'wp_head', 'wp_shortlink_wp_head', 10, 0 );

	if ( is_single() && ! machina_get_option( 'comments_posts' ) )
		remove_action( 'wp_head', 'feed_links_extra', 3 );

	if ( is_page() && ! machina_get_option( 'comments_pages' ) )
		remove_action( 'wp_head', 'feed_links_extra', 3 );

}

add_action( 'machina_meta', 'machina_seo_meta_description' );
/**
 ...
 * @global WP_Query $wp_query Query object
 */
function machina_seo_meta_description() {
	...
	if ( is_front_page() )
		$description = machina_get_seo_option( 'home_description' ) ? machina_get_seo_option( 'home_description' ) : get_bloginfo( 'description' );

	//* If we're on a single post / page / attachment
	if ( is_singular() ) {
		//* Description is set via custom field
		if ( machina_get_custom_field( '_machina_description' ) )
			$description = machina_get_custom_field( '_machina_description' );
		//* All-in-One SEO Pack (latest, vestigial)
		elseif ( machina_get_custom_field( '_aioseop_description' ) )
			$description = machina_get_custom_field( '_aioseop_description' );
		//* Headspace2 (vestigial)
		elseif ( machina_get_custom_field( '_headspace_description' ) )
			$description = machina_get_custom_field( '_headspace_description' );
		//* Thesis (vestigial)
		elseif ( machina_get_custom_field( 'thesis_description' ) )
			$description = machina_get_custom_field( 'thesis_description' );
		//* All-in-One SEO Pack (old, vestigial)
		elseif ( machina_get_custom_field( 'description' ) )
			$description = machina_get_custom_field( 'description' );
	}
	...
	if ( is_post_type_archive() && machina_has_post_type_archive_support() ) {
		$description = machina_get_cpt_option( 'description' ) ? machina_get_cpt_option( 'description' ) : '';
	}
...
add_action( 'machina_meta', 'machina_seo_meta_keywords' );
/**
 ...
 * @global WP_Query $wp_query Query object
 */
function machina_seo_meta_keywords() {
	...
	//* If we're on the home page
	if ( is_front_page() )
		$keywords = machina_get_seo_option( 'home_keywords' );

	//* If we're on a single post, page or attachment
	if ( is_singular() ) {
		//* Keywords are set via custom field
		if ( machina_get_custom_field( '_machina_keywords' ) )
			$keywords = machina_get_custom_field( '_machina_keywords' );
		//* All-in-One SEO Pack (latest, vestigial)
		elseif ( machina_get_custom_field( '_aioseop_keywords' ) )
			$keywords = machina_get_custom_field( '_aioseop_keywords' );
		//* Thesis (vestigial)
		elseif ( machina_get_custom_field( 'thesis_keywords' ) )
			$keywords = machina_get_custom_field( 'thesis_keywords' );
		//* All-in-One SEO Pack (old, vestigial)
		elseif ( machina_get_custom_field( 'keywords' ) )
			$keywords = machina_get_custom_field( 'keywords' );
	}
	...
	if ( is_post_type_archive() && machina_has_post_type_archive_support() ) {
		$keywords = machina_get_cpt_option( 'keywords' ) ? machina_get_cpt_option( 'keywords' ) : '';
	}
...
add_action( 'machina_meta', 'machina_robots_meta' );
/**
...
function machina_robots_meta() {
	...
	$meta = array(
		'noindex'   => '',
		'nofollow'  => '',
		'noarchive' => machina_get_seo_option( 'noarchive' ) ? 'noarchive' : '',
		'noodp'     => machina_get_seo_option( 'noodp' ) ? 'noodp' : '',
		'noydir'    => machina_get_seo_option( 'noydir' ) ? 'noydir' : '',
	);

	//* Check home page SEO settings, set noindex, nofollow and noarchive
	if ( is_front_page() ) {
		$meta['noindex']   = machina_get_seo_option( 'home_noindex' ) ? 'noindex' : $meta['noindex'];
		$meta['nofollow']  = machina_get_seo_option( 'home_nofollow' ) ? 'nofollow' : $meta['nofollow'];
		$meta['noarchive'] = machina_get_seo_option( 'home_noarchive' ) ? 'noarchive' : $meta['noarchive'];
	}
	...
	if ( is_category() ) {
		...
		$meta['noindex']   = machina_get_seo_option( 'noindex_cat_archive' ) ? 'noindex' : $meta['noindex'];
		$meta['noarchive'] = machina_get_seo_option( 'noarchive_cat_archive' ) ? 'noarchive' : $meta['noarchive'];

		//* noindex paged archives, if canonical archives is off
		$paged = get_query_var( 'paged' ) ? get_query_var( 'paged' ) : 1;
		$meta['noindex'] = $paged > 1 && ! machina_get_seo_option( 'canonical_archives' ) ? 'noindex' : $meta['noindex'];
	}
	...
	if ( is_tag() ) {
		...
		$meta['noindex']   = machina_get_seo_option( 'noindex_tag_archive' ) ? 'noindex' : $meta['noindex'];
		$meta['noarchive'] = machina_get_seo_option( 'noarchive_tag_archive' ) ? 'noarchive' : $meta['noarchive'];
	...
	if ( is_post_type_archive() && machina_has_post_type_archive_support() ) {
		$meta['noindex']   = machina_get_cpt_option( 'noindex' ) ? 'noindex' : $meta['noindex'];
		$meta['nofollow']  = machina_get_cpt_option( 'nofollow' ) ? 'nofollow' : $meta['nofollow'];
		$meta['noarchive'] = machina_get_cpt_option( 'noarchive' ) ? 'noarchive' : $meta['noarchive'];
	...
	if ( is_author() ) {
		...
		$meta['noindex']   = machina_get_seo_option( 'noindex_author_archive' ) ? 'noindex' : $meta['noindex'];
		$meta['noarchive'] = machina_get_seo_option( 'noarchive_author_archive' ) ? 'noarchive' : $meta['noarchive'];
	...
	if ( is_date() ) {
		$meta['noindex']   = machina_get_seo_option( 'noindex_date_archive' ) ? 'noindex' : $meta['noindex'];
		$meta['noarchive'] = machina_get_seo_option( 'noarchive_date_archive' ) ? 'noarchive' : $meta['noarchive'];
	}

	if ( is_search() ) {
		$meta['noindex']   = machina_get_seo_option( 'noindex_search_archive' ) ? 'noindex' : $meta['noindex'];
		$meta['noarchive'] = machina_get_seo_option( 'noarchive_search_archive' ) ? 'noarchive' : $meta['noarchive'];
	}

	if ( is_singular() ) {
		$meta['noindex']   = machina_get_custom_field( '_machina_noindex' ) ? 'noindex' : $meta['noindex'];
		$meta['nofollow']  = machina_get_custom_field( '_machina_nofollow' ) ? 'nofollow' : $meta['nofollow'];
		$meta['noarchive'] = machina_get_custom_field( '_machina_noarchive' ) ? 'noarchive' : $meta['noarchive'];
	}
...
add_action( 'machina_meta', 'machina_responsive_viewport' );
/**
 * Optionally output the responsive CSS viewport tag.
 *
 * Child theme needs to support `machina-responsive-viewport`.
 *
 * @since 1.9.0
 *
 * @return null Return early if child theme does not support viewport.
 */
function machina_responsive_viewport() {

	if ( ! current_theme_supports( 'machina-responsive-viewport' ) )
		return;
...
add_action( 'wp_head', 'machina_load_favicon' );
/**
 * Echo favicon link if one is found.
 *
 * Falls back to Machina theme favicon.
 *
 * URL to favicon is filtered via `machina_favicon_url` before being echoed.
 *
 * @since 0.2.2
 *
 * @uses CHILD_DIR
 ...
 * @uses PARENT_URL
 */
function machina_load_favicon() {

	//* Allow child theme to short-circuit this function
	$pre = apply_filters( 'machina_pre_load_favicon', false );
	...
	$favicon = apply_filters( 'machina_favicon_url', $favicon );
...
add_action( 'wp_head', 'machina_canonical', 5 );
/**
...
function machina_canonical() {
	...
	if ( is_singular() ) {
		...
		$cf = machina_get_custom_field( '_machina_canonical_uri' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$canonical = machina_get_seo_option( 'canonical_archives' ) ? get_term_link( (int) $id, $taxonomy ) : 0;
	}
	...
	if ( is_author() ) {
		...
		$canonical = machina_get_seo_option( 'canonical_archives' ) ? get_author_posts_url( $id ) : 0;
	}

	if ( $canonical )
		printf( '<link rel="canonical" href="%s" />' . "\n", esc_url( apply_filters( 'machina_canonical', $canonical ) ) );

}

add_action( 'wp_head', 'machina_rel_author' );
/**
 ...
 */
function machina_rel_author() {

	if ( is_front_page() && $gplus_url = get_user_option( 'googleplus', machina_get_seo_option( 'home_author' ) ) ) {
...
add_filter( 'machina_header_scripts', 'do_shortcode' );
add_action( 'wp_head', 'machina_header_scripts' );
/**
 * Echo header scripts in to wp_head().
 *
 * Allows shortcodes.
 *
 * Applies `machina_header_scripts` filter on value stored in header_scripts setting.
 *
 ...
 *
 * @uses machina_get_option()       Get theme setting value.
 * @uses machina_get_custom_field() Echo custom field value.
 */
function machina_header_scripts() {

	echo apply_filters( 'machina_header_scripts', machina_get_option( 'header_scripts' ) );

	//* If singular, echo scripts from custom field
	if ( is_singular() )
		machina_custom_field( '_machina_scripts' );

}

add_action( 'after_setup_theme', 'machina_custom_header' );
/**
 ...
 *
 * Applies `machina_custom_header_defaults` filter.
 *
 * @since 1.6.0
 *
 * @return null Return early if custom header not supported in the theme.
 */
function machina_custom_header() {

	$custom_header = get_theme_support( 'machina-custom-header' );
	$wp_custom_header = get_theme_support( 'custom-header' );

	//* If not active (Machina of WP custom header), do nothing
	if ( ! $custom_header && ! $wp_custom_header )
		return;

	//* Blog title option is obsolete when custom header is active
	add_filter( 'machina_pre_get_option_blog_title', '__return_empty_array' );
	...
	$args = wp_parse_args(
		$custom_header,
		apply_filters(
			'machina_custom_header_defaults',
			array(
			'width'                 => 960,
			'height'                => 80,
			'textcolor'             => '333333',
...
add_action( 'wp_head', 'machina_custom_header_style' );
/**
 ...
 */
function machina_custom_header_style() {
...
add_action( 'machina_header', 'machina_header_markup_open', 5 );
/**
 ...
 * @uses machina_structural_wrap() Maybe add opening .wrap div tag with header context.
 */
function machina_header_markup_open() {
	...
	machina_structural_wrap( 'header' );

}

add_action( 'machina_header', 'machina_header_markup_close', 15 );
/**
 * Echo the opening structural markup for the header.
 *
 * @since 1.2.0
 *
 * @uses machina_structural_wrap() Maybe add closing .wrap div tag with header context.
 * @uses machina_markup()          Apply contextual markup.
 */
function machina_header_markup_close() {
...
add_action( 'machina_header', 'machina_do_header' );
/**
 * Echo the default header, including the #title-area div, along with #title and #description, as well as the .widget-area.
 *
 * Does the `machina_site_title`, `machina_site_description` and `machina_header_right` actions.
 *
 * @since 1.0.2
 *
...
function machina_do_header() {
	...
	if ( ( isset( $wp_registered_sidebars['header-right'] ) && is_active_sidebar( 'header-right' ) ) || has_action( 'machina_header_right' ) ) {
		machina_markup( array(
			...
			add_filter( 'wp_nav_menu_args', 'machina_header_menu_args' );
			add_filter( 'wp_nav_menu', 'machina_header_menu_wrap' );
			dynamic_sidebar( 'header-right' );
			remove_filter( 'wp_nav_menu_args', 'machina_header_menu_args' );
...
add_action( 'machina_site_title', 'machina_seo_site_title' );
/**
 * Echo the site title into the header.
 *
 * Depending on the SEO option set by the user, this will either be wrapped in an `h1` or `p` element.
 *
 * Applies the `machina_seo_title` filter before echoing.
 *
 * @since 1.1.0
 *
 * @uses machina_get_seo_option() Get SEO setting value.
...
function machina_seo_site_title() {
	...
	$wrap = is_home() && 'title' === machina_get_seo_option( 'home_h1_on' ) ? 'h1' : 'p';
...
add_action( 'machina_site_description', 'machina_seo_site_description' );
/**
 * Echo the site description into the header.
 *
 * Depending on the SEO option set by the user, this will either be wrapped in an `h1` or `p` element.
 *
 * Applies the `machina_seo_description` filter before echoing.
 *
 * @since 1.1.0
 *
 * @uses machina_get_seo_option() Get SEO setting value.
...
/**
 * Sets a common class, `.machina-nav-menu`, for the custom menu widget if used in the header right sidebar.
 *
 * @since 1.9.0
 *
 * @uses machina_html5() Check for HTML5 support.
