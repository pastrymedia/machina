 ...
 *
 * @package Machina\Archives
 * @author  MachinaThemes
 * @license GPL-2.0+
 * @link    http://my.machinathemes.com/themes/machina/
 */

add_filter( 'machina_term_intro_text_output', 'wpautop' );
add_action( 'machina_before_loop', 'machina_do_taxonomy_title_description', 15 );
/**
 ...
 */
function machina_do_taxonomy_title_description() {
	...
	if ( $term->meta['intro_text'] )
		$intro_text = apply_filters( 'machina_term_intro_text_output', $term->meta['intro_text'] );
...
add_filter( 'machina_author_intro_text_output', 'wpautop' );
add_action( 'machina_before_loop', 'machina_do_author_title_description', 15 );
/**
 ...
 */
function machina_do_author_title_description() {
	...
	$intro_text = $intro_text ? apply_filters( 'machina_author_intro_text_output', $intro_text ) : '';
...
add_action( 'machina_before_loop', 'machina_do_author_box_archive', 15 );
/**
 ...
 * @since 1.4.0
 *
 * @uses machina_author_box() Echo the author box and its contents.
 *
 * @see machina_do_author_title_and_description Author title and description.
 *
 * @return Return early if not author archive or not page one.
 */
function machina_do_author_box_archive() {

	if ( ! is_author() || get_query_var( 'paged' ) >= 2 )
		return;

	if ( get_the_author_meta( 'machina_author_box_archive', get_query_var( 'author' ) ) )
		machina_author_box( 'archive' );

}

add_filter( 'machina_cpt_archive_intro_text_output', 'wpautop' );
add_action( 'machina_before_loop', 'machina_do_cpt_archive_title_description' );
/**
 ...
 * @since 2.0.0
 *
 * @uses machina_has_post_type_archive_support() Check if a post type should potentially support an archive setting page.
 * @uses machina_get_cpt_option()                Get list of custom post types which need an archive settings page.
 *
 * @return null Return early if not on relevant post type archive.
 */
function machina_do_cpt_archive_title_description() {
	...
	$headline   = machina_get_cpt_option( 'headline' );
	$intro_text = machina_get_cpt_option( 'intro_text' );

	$headline   = $headline ? sprintf( '<h1 class="archive-title">%s</h1>', $headline ) : '';
	$intro_text = $intro_text ? apply_filters( 'machina_cpt_archive_intro_text_output', $intro_text ) : '';
