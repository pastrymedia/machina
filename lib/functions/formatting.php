 ...
 */
function machina_truncate_phrase( $text, $max_characters ) {
...
function get_the_content_limit( $max_characters, $more_link_text = '(more...)', $stripteaser = false ) {
	...
	$content = machina_truncate_phrase( $content, $max_characters );
 ...
 * @since 1.0.0
 *
 * @uses machina_strip_attr() Remove any existing rel attribute from links.
 *
 * @param string $text HTML markup.
 *
 * @return string Amendment HTML markup.
 */
function machina_rel_nofollow( $text ) {

	$text = machina_strip_attr( $text, 'a', 'rel' );
 ...
 */
function machina_strip_attr( $text, $elements, $attributes, $two_passes = true ) {
 ...
 */
function machina_sanitize_html_classes( $classes, $return_format = 'input' ) {
 ...
 */
function machina_formatting_allowedtags() {

	return apply_filters(
		'machina_formatting_allowedtags',
		array(
 ...
 * @since 1.8.0
 *
 * @uses machina_formatting_allowedtags() List of allowed HTML elements.
 *
 * @param string $string Content to filter through kses.
 *
 * @return string
 */
function machina_formatting_kses( $string ) {

	return wp_kses( $string, machina_formatting_allowedtags() );
 ...
 */
function machina_human_time_diff( $older_date, $newer_date = false ) {
	...
	if ( ! $since )
		return '0 ' . _x( 'seconds', 'time difference', 'machina' );
	...
	$output = sprintf( translate_nooped_plural( $units[$i][1], $count, 'machina' ), $count );
	...
	if ( $ii < $j ) {
		...
		if ( ( $count2 = floor( ( $since - ( $seconds * $count ) ) / $seconds2 ) ) !== 0 )
			...
			$output .= sprintf( ' %s ' . translate_nooped_plural( $units[$ii][1], $count2, 'machina' ),	_x( 'and', 'separator in time difference', 'machina' ),	$count2	);
	}
 ...
 *
 * Used almost exclusively within labels and text in user interfaces added by Machina.
 *
 * @since 2.0.0
 *
