 ...
 *
 * Applies `machina_pre_get_option_$key` and `machina_options` filters.
 *
 ...
 * @param string  $setting    Optional. Settings field name. Eventually defaults to `MACHINA_SETTINGS_FIELD` if not
 *                            passed as an argument.
 * @param boolean $use_cache  Optional. Whether to use the Machina cache value or not. Default is true.
 *
 * @return mixed The value of this $key in the database.
 */
function machina_get_option( $key, $setting = null, $use_cache = true ) {


	//* The default is set here, so it doesn't have to be repeated in the function arguments for machina_option() too.
	$setting = $setting ? $setting : MACHINA_SETTINGS_FIELD;
	...
	$pre = apply_filters( 'machina_pre_get_option_' . $key, null, $setting );
	if ( null !== $pre )
		return $pre;
	...
	if ( isset( $settings_cache[$setting] ) )
		...
		$options = apply_filters( 'machina_options', $settings_cache[$setting], $setting );
	else
		//* Set value and cache setting
		$options = $settings_cache[$setting] = apply_filters( 'machina_options', get_option( $setting ), $setting );
 ...
 * @since 0.1.3
 *
 * @uses machina_get_option() Return option from the options table and cache result.
 *
 * @param string  $key       Option name.
 * @param string  $setting   Optional. Settings field name. Eventually defaults to MACHINA_SETINGS_FIELD.
 * @param boolean $use_cache Optional. Whether to use the Machina cache value or not. Default is true.
 */
function machina_option( $key, $setting = null, $use_cache = true ) {

	echo machina_get_option( $key, $setting, $use_cache );
 ...
 *
 * @param string  $key       Option name.
 * @param boolean $use_cache Optional. Whether to use the Machina cache value or not. Defaults to true.
 *
 * @return mixed The value of this $key in the database.
 */
function machina_get_seo_option( $key, $use_cache = true ) {

	return machina_get_option( $key, MACHINA_SEO_SETTINGS_FIELD, $use_cache );
 ...
 * @since 0.1.3
 *
 * @uses machina_option() Echo option from the options table and cache result.
 * @uses MACHINA_SEO_SETTINGS_FIELD
 *
 * @param string  $key       Option name.
 * @param boolean $use_cache Optional. Whether to use the Machina cache value or not. Defaults to true.
 */
function machina_seo_option( $key, $use_cache = true ) {

	machina_option( $key, MACHINA_SEO_SETTINGS_FIELD, $use_cache );
 ...
 * @uses machina_get_option()                      Return option from the options table and cache result.
 * @uses MACHINA_CPT_ARCHIVE_SETTINGS_FIELD_PREFIX
 *
 * @param string $key            Option name.
 * @param string $post_type_name Post type name.
 * @param bool   $use_cache      Optional. Whether to use the Machina cache value or not. Defaults to true.
 *
 * @return mixed The option value.
 */
function machina_get_cpt_option( $key, $post_type_name = '', $use_cache = true ) {

	$post_type_name = machina_get_global_post_type_name( $post_type_name );

	return machina_get_option( $key, MACHINA_CPT_ARCHIVE_SETTINGS_FIELD_PREFIX . $post_type_name, $use_cache );
 ...
 * @since 2.0.0
 *
 * @uses machina_get_cpt_option() Return a CPT Archive setting value from the options table.
 *
 * @param string $key            Option name.
 * @param string $post_type_name Post type name.
 * @param bool   $use_cache      Optional. Whether to use the Machina cache value or not. Defaults to true.
 */
function machina_cpt_option( $key, $post_type_name, $use_cache = true ) {

	echo machina_get_cpt_option( $key, $post_type_name, $use_cache );
 ...
 * @since 0.1.3
 *
 * @uses machina_get_custom_field() Return custom field post meta data.
 *
 * @param string $field          Custom field key.
 * @param string $output_pattern printf() compatible output pattern.
 */
function machina_custom_field( $field, $output_pattern = '%s' ) {

	if ( $value = machina_get_custom_field( $field ) )
 ...
 */
function machina_get_custom_field( $field ) {
 ...
 */
function machina_save_custom_fields( array $data, $nonce_action, $nonce_name, $post, $deprecated = null ) {
...
add_filter( 'get_term', 'machina_get_term_filter', 10, 2 );
/**
 * Merge term meta data into options table.
 *
 * Machina is forced to create its own term-meta data structure in the options table, since it is not support in core WP.
 *
 * Applies `machina_term_meta_defaults`, `machina_term_meta_{field}` and `machina_term_meta` filters.
 *
 * @since 1.2.0
 *
 ...
 */
function machina_get_term_filter( $term, $taxonomy ) {

	//* Do nothing, if $term is not object
	if ( ! is_object( $term ) )
		return $term;

	$db = get_option( 'machina-term-meta' );
	$term_meta = isset( $db[$term->term_id] ) ? $db[$term->term_id] : array();

	$term->meta = wp_parse_args( $term_meta, apply_filters( 'machina_term_meta_defaults', array(
		'headline'            => '',
		'intro_text'          => '',
	...
	foreach ( $term->meta as $field => $value )
		$term->meta[$field] = apply_filters( 'machina_term_meta_' . $field, stripslashes( wp_kses_decode_entities( $value ) ), $term, $taxonomy );

	$term->meta = apply_filters( 'machina_term_meta', $term->meta, $term, $taxonomy );

	return $term;

}

add_filter( 'get_terms', 'machina_get_terms_filter', 10, 2 );
/**
 * Add Machina term-meta data to functions that return multiple terms.
 *
 * @since 2.0.0
 *
 ...
 */
function machina_get_terms_filter( array $terms, $taxonomy ) {

	foreach( $terms as $term )
		$term = machina_get_term_filter( $term, $taxonomy );
 ...
 * @param string       $setting Optional. Settings field name. Default is MACHINA_SETTINGS_FIELD.
 */
function _machina_update_settings( $new = '', $setting = MACHINA_SETTINGS_FIELD ) {
