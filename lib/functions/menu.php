...
/**
 * Determine if a child theme supports a particular Machina nav menu.
 *
 * @since 1.8.0
 *
 ...
 */
function machina_nav_menu_supported( $menu ) {
 ...
 * `machina_superfish_enabled` filter is true, then this function returns true. False otherwise.
 *
 * @since 1.9.0
 *
 * @uses machina_html5()      Check for HTML5 support.
 * @uses machina_get_option() Get Theme settings value.
 *
 * @return boolean True if Superfish is enabled, false otherwise.
 */
function machina_superfish_enabled() {

	return ( ! machina_html5() && machina_get_option( 'superfish' ) ) || apply_filters( 'machina_superfish_enabled', false );
 ...
 *
 * Now only used for backwards-compatibility (machina_vestige).
 *
 ...
 *  - link_after     => ''
 *
 * Themes can short-circuit the function early by filtering on `machina_pre_nav` or on the string of list items via
 * `machina_nav_items`. They can also filter the complete menu markup via `machina_nav`. The `$args` (merged with
 * defaults) are available for all filters.
 *
 * @since 0.2.3
 *
 * @uses machina_get_seo_option() Get SEO setting value.
 * @uses machina_rel_nofollow()   Add `rel="nofollow"` attribute and value to all links.
 *
 * @see machina_do_nav()
 * @see machina_do_subnav()
 *
 * @param array $args Menu arguments.
 *
 * @return string HTML for menu, unless `machina_pre_nav` returns something truthy.
 */
function machina_nav( $args = array() ) {

	if ( isset( $args['context'] ) )
		_deprecated_argument( __FUNCTION__, '1.2', __( 'The argument, "context", has been replaced with "theme_location" in the $args array.', 'machina' ) );
	...
	$defaults = apply_filters( 'machina_nav_default_args', $defaults );
	$args     = wp_parse_args( $args, $defaults );

	//* Allow child theme to short-circuit this function
	$pre = apply_filters( 'machina_pre_nav', false, $args );
	if ( $pre )
		return $pre;
	...
	if ( isset( $args['show_home'] ) && ! empty( $args['show_home'] ) ) {
		if ( true === $args['show_home'] || '1' === $args['show_home'] || 1 === $args['show_home'] )
			$text = apply_filters( 'machina_nav_home_text', __( 'Home', 'machina' ), $args );
		else
			$text = $args['show_home'];
		...
		$menu .= machina_get_seo_option( 'nofollow_home_link' ) ? machina_rel_nofollow( $home ) : $home;
	...
	$menu = apply_filters( 'machina_nav_items', $menu, $args );
	...
	$menu = apply_filters( 'machina_nav', $menu, $args );
