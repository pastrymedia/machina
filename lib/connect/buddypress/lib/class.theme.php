<?php
/*
	This is the buddypress specific code to manage the buddypress content being added to the Machina Framework
...
class GConnect_Theme {
	...
	function __construct( $tp_active = false ) {
		...
		if( !( $this->style = machina_get_option( 'style_selection' ) ) )
		...
		if ( ! is_admin() || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
			...
			require_once( MACHINACONNECT_DIR . 'lib/class.front.php' );
			$this->front = new GConnect_Front( $this, $tp_active );

		}
		if( is_admin() ) {
			require_once( MACHINACONNECT_DIR . 'lib/class.options.php' );
			...
			if( $custom_register && 'none' != $custom_register ) {
				require_once( MACHINACONNECT_DIR . 'lib/class.visitor.php' );
		...
		if( 'widget' == $this->get_option( 'before_content' ) ) {
			machina_register_sidebar(array(
				...
				'description' => __( 'This is above the BuddyPress content', 'machina-connect' ),
				'id' => 'gconnect-before'
			));
		}

		if( is_dir( MACHINACONNECT_DIR . 'child-theme/' . $this->stylesheet ) ) {
			$addon_directory = MACHINACONNECT_DIR . 'child-theme/' . $this->stylesheet;
			$addon_functions = $addon_directory . '/my-functions.php';
			if( is_file( $addon_functions ) )
				require_once( $addon_functions );
			if( !is_admin() )
				$this->front->set_addon( $addon_directory, MACHINACONNECT_URL . 'child-theme/' . $this->stylesheet );
		} elseif( !is_admin() )
...
class GConnect_LoginWidget extends WP_Widget {
	function GConnect_LoginWidget() {
		$widget_ops = array( 'description' => __( 'Machina Connect Login Widget', 'machina-connect' ) );
		$this->WP_Widget( 'gc_login_widget', __( 'Machina Connect Login Widget', 'machina-connect' ), $widget_ops );
	}
...
<?php		}
		$gconnect_theme->front->machina_before_content( $wrap );
		if( $wrap ) { ?>
		<div class="padder">
<?php		}
	}
...
?>
	...
	<?php do_action( 'machina_after_content' ); ?>
...
function gconnect_get_header() {
	...
	echo "\n\t\t<div id=\"content-sidebar-wrap\">\n";
	$gconnect_theme->front->machina_before_content( 'true' );
...
function gconnect_GB_admin_notice() { ?>
		<div id="message" class="updated fade">
			<p><?php printf( __( 'Please <a href="%s">switch</a> to a non-MachinaBuddy child theme to enable the BuddyPress features in MachinaConnect.', 'machina-connect' ), admin_url( 'themes.php' ) ) ?></p>
		</div>
<?php	}
if( function_exists( 'rabp_machina_css' ) ) {
...
/*
 * All functions below are included for backward compatability with MachinaBuddy
 */
function rabp_machina_site_nav() {
	gconnect_site_nav();
}
function rabp_machina_body_class( $classes = array() ) {
	return gconnect_body_class( $classes );
}
function machinabuddy_before_content() {
	gconnect_before_content( false );
}
function rabp_machina_have_bp_adminbar() {
	return gconnect_have_adminbar();
}
function rabp_machina_have_bp() {
	global $bp;
	return isset( $bp );
}
function rabp_machina_is_bp_home() {
	return gconnect_is_bp_home();
}
function rabp_machina_is_blog_page() {
	return bp_is_blog_page();
}
function rabp_machina_get_bp_signup() {
	return gconnect_get_bp_signup();
}
/*
 * add clear divs to directory pages
 */
function rabp_machina_add_clear() { ?>
		<div class="clear"></div>
<?php
}
add_action( 'bp_after_member_header', 'rabp_machina_add_clear' );
add_action( 'bp_after_group_header', 'rabp_machina_add_clear' );
}
