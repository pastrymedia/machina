<?php
// This class contains the code only used with visitors in Machina Connect
...
class GConnect_visitor {
	...
	function signup_time_check( $result ) {
		...
		if( substr( $_POST[ 'gconnect_signup_hash' ], 4 ) != $this->signup_hash( $hextime ) || $this->time - $orig_time < $this->signup_min ) {
			$result['errors']->add( 'gconnect_time', __('Please try again', 'machina-connect' ) );
	...
	function bp_signup_validate() {
		global $bp;
		if( isset( $bp ) )
			$bp->signup->errors['gconnect_time'] = __( 'Please try again', 'machina-connect' );
	}
	...
	function signup_redirect() {
		...
		if( substr( $_SERVER[ 'REQUEST_URI' ], 0, strlen( $register_slug ) ) == $register_slug ) {
			...
			add_action( 'machina_meta', array( &$this, 'signup_noindex' ), 9 );
		}
	}
	function signup_noindex() {
		remove_action('machina_meta','machina_index_follow_logic');
