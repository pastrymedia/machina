...
class GConnect_Front {
	...
	function  __construct( &$theme, $tp_active = false ) {
		...
		add_action( 'machina_meta', array( &$this, 'machina_meta' ), 2 );
		...
		add_filter( 'machina_pre_get_option_site_layout', array( &$this, 'site_layout' ), 20 );
	...
	function machina_meta() {
		...
		if( $this->theme->have_adminbar() && $this->adminbar )
			wp_enqueue_style( 'gconnect-adminbar', MACHINACONNECT_URL . 'css/adminbar.css', MACHINACONNECT_VERSION );
		if( $this->theme->is_home() ) {
			wp_enqueue_style( 'gconnect-bp', MACHINACONNECT_URL . 'css/buddypress.css', MACHINACONNECT_VERSION );

			wp_enqueue_script( 'bp-legacy-js', MACHINACONNECT_URL . 'lib/global.js', array( 'jquery' ), MACHINACONNECT_VERSION );
			$params = array(
		...
		foreach( array( '-' . $this->theme->child_style(), '.css' ) as $f ) {
			...
			if( is_file( CHILD_DIR . $maybe_css ) ) {
				...
				wp_enqueue_style( 'gconnect-cust' . str_replace( '.css', '', $f ), CHILD_URL . $maybe_css, defined( 'CHILD_THEME_VERSION' ) ? CHILD_THEME_VERSION : MACHINACONNECT_VERSION );
				break;
			...
			if( is_file( $this->addon_directory . $maybe_css ) ) {
				...
				wp_enqueue_style( 'gconnect-cust' . str_replace( '.css', '', $f ), $this->addon_url . $maybe_css, MACHINACONNECT_VERSION );
				break;
	...
	function after_setup_theme() {
		if( ! function_exists( 'bp_dtheme_ajax_querystring' ) )
			require_once( MACHINACONNECT_DIR . 'lib/ajax.php' );
	}
	function init() {
		if( is_admin() )
			return;
...
//		wp_deregister_style( 'bp-admin-bar' );
	...
	function wp_head() {
		...
		if( $this->theme->have_adminbar() ) {
			...
			if( $this->adminbar )
				add_action( 'machina_after_footer', 'bp_core_admin_bar', 88 );
		...
		if( $this->get_option( 'login_sidebar' ) )
			add_action('machina_before_sidebar_widget_area', array( &$this, 'sidebar' ) );
		...
		if( ( $do_sidebars = defined( 'SS_SETTINGS_FIELD' ) ) || $gconnect_theme->do_custom_subnav() ) {
			if( $gconnect_theme->do_custom_subnav() && ( $gconnect_theme->custom_subnav->menu = $this->get_option( $this->key . 'subnav' ) ) ) {
				add_filter( 'machina_pre_get_option_subnav_type', array( &$gconnect_theme->custom_subnav, 'pre_get_option_subnav_type' ) );
			...
			if( $do_sidebars ) {
				...
				add_action('machina_sidebar', array( &$this, 'machina_do_sidebar' ) );
				add_action('machina_sidebar_alt', array( &$this, 'machina_do_sidebar' ) );
			}
		}
	}
	function sidebar() {
	...
	<?php else : ?>
		<?php do_action( 'bp_before_sidebar_login_form' ) ?>
			...
			<form name="login-form" id="login-form" class="standard-form" action="<?php echo site_url( 'wp-login.php', 'login' ) ?>" method="post">
				...
				<input type="submit" name="wp-submit" id="userbar_wp-submit" value="<?php _e( 'Log In', 'machina-connect' ); ?>" tabindex="100" />
	...
	function machina_before_content( $wrap = false ) {
		...
		if( 'widget' == $before ) { ?>
				<div class="bp-content-top">
					<?php if( !dynamic_sidebar( 'gconnect-before' ) ) : ?>
						<div class="widget">
							<h4><?php _e( "Before BuddyPress (Optional)", 'machina-connect' ); ?></h4>
							<div class="wrap">
								<p><?php _e( "This is an optional widgeted area which is called Before BuddyPress. To get started, log into your WordPress dashboard, and then go to the Machina > Theme Settings. In the BuddyPress Settings choose Widget Area on the BuddyPress Before Content dropdown. Then go to Appearance > Widgets screen. There you can drag any widget into the Before BuddyPress widget area on the right hand side. ", 'machina-connect' ); ?></p>
							</div><!-- end .wrap -->
						</div><!-- end .widget -->
					<?php endif; ?>
	...
	function wp_list_pages( $menu ) {
		if( $this->theme->is_home() && bp_get_signup_allowed() && !empty( $this->visitor ) ) {
			if( 'before_pages' == $this->visitor->get_register() )
				$menu = rabp_machina_get_bp_signup() . $menu;
			elseif( 'after_pages' == $this->visitor->get_register() )
				$menu .= rabp_machina_get_bp_signup();
		}
		return $menu;
	}
	...
	function locate_template( $template, $default = '' ) {
		$maybe = MACHINACONNECT_DIR . 'templates/' . $template;
