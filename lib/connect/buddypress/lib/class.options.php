...
class GConnect_Admin {
	...
	function admin_notice() { ?>
		<div id="message" class="updated fade">
			<p><?php printf( __( 'Please <a href="%s">deactivate</a> the BP Template Pack plugin to enable the BuddyPress features in MachinaConnect.', 'machina-connect' ), admin_url( 'plugins.php' ) ) ?></p>
		</div>
<?php	}
	function admin_init() {
	...
	function admin_menu() {
		...
		$this->pagehook = add_submenu_page( 'machina', __( 'Connect Settings', 'machina-connect' ), __( 'Connect Settings', 'machina-connect' ), $cap, 'connect-settings', array( &$this, 'theme_options' ) );
		add_action( 'load-' . $this->pagehook, array( &$this, 'add_metabox' ) );
	}
	function add_metabox() {
		add_meta_box('connect-theme-settings-nav', __('Navigation', 'machina-connect'), array( &$this, 'metabox_navigation' ), $this->pagehook, 'column1');
		add_meta_box('connect-theme-settings-registration', __('Registration', 'machina-connect'), array( &$this, 'metabox_registration' ), $this->pagehook, 'column1');
		add_meta_box('connect-theme-settings-layout', __('Layout', 'machina-connect'), array( &$this, 'metabox_layout' ), $this->pagehook, 'column1');
	}
...
?>
<div id="connect-theme-settings" class="wrap machina-metaboxes">
...
<?php		wp_nonce_field('closedpostboxes', 'closedpostboxesnonce', false );
	...
	<h2>
		<?php _e('Machina - Connect Settings', 'machina-connect'); ?>
		<input type="submit" class="button-primary" value="<?php _e('Save Settings', 'machina-connect') ?>" />
	</h2>
...
?>
			<p><?php _e( 'BuddyPress Adminbar Navigation:', 'machina-connect' ); ?>
			<select name="<?php echo $this->theme->settings_key; ?>[adminbar]">
			<option value=""><?php _e( ' -- None -- ', 'machina-connect' ); ?></option>
<?php
			...
			<p><input type="checkbox" name="<?php echo $this->theme->settings_key; ?>[home_adminbar]" id="<?php echo $this->theme->settings_key; ?>[home_adminbar]" value="1" <?php checked(1, $this->get_option('home_adminbar')); ?> />
				<label for="<?php echo $this->theme->settings_key; ?>[home_adminbar]"><?php _e('Show the adminbar on the home page', 'machina-connect'); ?></label></p>
<?php
		}
?>
			<p><input type="checkbox" name="<?php echo $this->theme->settings_key; ?>[login_sidebar]" id="<?php echo $this->theme->settings_key; ?>[login_sidebar]" value="1" <?php checked(1, $this->get_option('login_sidebar')); ?> />
				<label for="<?php echo $this->theme->settings_key; ?>[login_sidebar]"><?php _e('Show the login form at the top of the main sidebar', 'machina-connect'); ?></label></p>
<?php
...
?>
			<p><input type="checkbox" name="<?php echo $this->theme->settings_key; ?>[login_adminbar]" id="<?php echo $this->theme->settings_key; ?>[login_adminbar]" value="1" <?php checked(1, $this->get_option('login_adminbar')); ?> />
				<label for="<?php echo $this->theme->settings_key; ?>[login_adminbar]"><?php _e('Show the login link in the adminbar', 'machina-connect'); ?></label></p>
<?php
		}
	}
	function metabox_layout() {
		...
		$custom_subnav = array( 'visitor' => __( 'Visitors', 'machina-connect' ), 'user' => __( 'Logged in Users', 'machina-connect' ) );
		...
		_e( 'Show activity on the front page', 'machina-connect' );
		?></label></p>
		...
		_e( 'Full Width Content for Group Forum layout', 'machina-connect' );
		?></label></p>
			<p><?php _e('BuddyPress Before Content:', 'machina-connect'); ?>
			<select name="<?php echo $this->theme->settings_key; ?>[before_content]">
				<option style="padding-right:10px;" value="none" <?php selected( 'none', $bp_before_content ); ?>><?php _e('None', 'machina-connect'); ?></option>
				<option style="padding-right:10px;" value="widget" <?php selected( 'widget', $bp_before_content ); ?>><?php _e('Widget Area', 'machina-connect'); ?></option>
				<option style="padding-right:10px;" value="machina" <?php selected( 'machina', $bp_before_content ); ?>><?php _e('Machina Before Content', 'machina-connect'); ?></option>
			</select></p>
	<?php	if( ( $do_sidebars = defined( 'SS_SETTINGS_FIELD' ) ) || $gconnect_theme->do_custom_subnav() ) {
			foreach( $custom_subnav as $k => $v ) {
				...
				if( $do_sidebars )
					$this->print_sidebar_select( array( $key . 'sidebar' => __( 'Primary Sidebar', 'machina-connect' ), $key . 'sidebar_alt' => __( 'Secondary Sidebar', 'machina-connect' ) ) );
				echo '</p>';
			}
		}
	}
	function metabox_registration() {
		if ( !bp_get_signup_allowed() ) {
			printf( '<p>' . __( '<a href="%s">Registration</a> is currently disabled', 'machina-connect' ) . '</p>', is_multisite() ? network_admin_url( 'settings.php' ) : admin_url( 'options-general.php' ) );
...
?>
			<p><?php _e('Custom Registration Permalink:', 'machina-connect'); ?>
			<select name="<?php echo $this->theme->settings_key; ?>[custom_register]">
				<option style="padding-right:10px;" value="none" <?php selected( 'none', $custom_register ); ?>><?php _e('None', 'machina-connect'); ?></option>
				<option style="padding-right:10px;" value="before_pages" <?php selected( 'before_pages', $custom_register ); ?>><?php _e('Before Pages', 'machina-connect'); ?></option>
				<option style="padding-right:10px;" value="after_pages" <?php selected( 'after_pages', $custom_register ); ?>><?php _e('After Pages', 'machina-connect'); ?></option>
				<option style="padding-right:10px;" value="adminbar" <?php selected( 'adminbar', $custom_register ); ?>><?php _e('On the Adminbar', 'machina-connect'); ?></option>
			</select></p>
			<small><strong><?php _e( 'Using the custom registration disables the default BuddyPress registration page', 'machina-connect' ); ?></strong></small></p>
			<p><?php _e( 'Custom Permalink:', 'machina-connect' ); ?><br />
			...
			<small><strong><?php printf( __( "Don't include the %s", 'machina-connect' ), get_option( 'siteurl' ) ); ?></strong></small></p>

			<p><?php _e("Custom Title:", 'machina-connect'); ?><br />
			...
			<small><strong><?php _e("The title for the link", 'machina-connect'); ?></strong></small></p>

			<p><?php _e("Registration Time:", 'machina-connect'); ?><br />
			...
			<small><strong><?php _e("Minimum number of seconds for a human signup", 'machina-connect'); ?></strong></small></p>
<?php	}
	function page_metabox() {
		...
		<label for="<?php echo $this->activity_fieldname; ?>"><?php _e( 'Use this for the Recent Activity Page', 'machina-connect' ); ?></label></p>
<?php	}
	...
	?>
		<p>
			...
			<select name="<?php echo $this->theme->settings_key . "[$field_name]"; ?>" id="<?php echo $this->theme->settings_key . "[$field_name]"; ?>">
				<option style="padding-right:10px;" value=""><?php _e( 'Default', 'machina-connect' ); ?></option>
