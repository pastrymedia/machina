...
//add_action( 'machina_before_loop', 'woocommerce_breadcrumb', 10, 0 );
...
add_filter( 'machina_pre_get_option_site_layout', 'machinawooc_archive_layout' );
/**
 * Manage page layout for the Product archive (Shop) page
 *
 * Set the layout in the Machina layouts metabox in the Page Editor
 *
 * @since 0.9.0
 *
 * @param str $layout Machina layout, eg 'content-sidebar', etc
 * @global string|int $shop_page_id The ID of the Shop WP Page
 * @return str $layout Shop Page layout from postmeta
 */
function machinawooc_archive_layout( $layout ) {

	global $shop_page_id;

	$layout = get_post_meta( $shop_page_id, '_machina_layout', true );

	return $layout;
}

add_action( 'machina_before_loop', 'machinawooc_archive_product_loop' );
/**
 ...
 *
 * @uses machinawooc_content_product() if WooC is version 1.6.0+
 * @uses machinawooc_product_archive() for earlier WooC versions
 *
 * @since 0.9.0
 * @updated 0.9.4
 * @global object $woocommerce
 */
function machinawooc_archive_product_loop() {
	...
	if ( $new )
		machinawooc_content_product();

	else
		machinawooc_product_archive();
}
